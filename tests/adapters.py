...
def run_linear(
    ...
    my_linear = LinearLayer(d_in, d_out)
    state = {'weight': weights}
    my_linear.load_state_dict(state)
...
def run_embedding(
    ...
    from cs336_basics.model import TokenEmbedding
    my_embedding = TokenEmbedding(vocab_size, d_model)
    state = {'weight': weights}
    my_embedding.load_state_dict(state)
...
def run_scaled_dot_product_attention(
    ...
    sdp_attention = ScaledDotProductAttention(mask)
    return sdp_attention(Q, K, V)
...
def run_rope(
    ...
    output = RotaryPositionalEmbedding(theta, d_k, max_seq_len, device=in_query_or_key.device)
    return output(in_query_or_key, token_positions)
...
def run_transformer_block(
    ...
    state_dict = {
        'rmsnorm1.gain': weights['ln1.weight'],  # 根据你的 RMSNorm 实现调整
...
def run_transformer_lm(
    ...
    for i in range(num_layers):
        # 注意力权重
        state_dict[f'layers.{i}.mha.w_q.weight'] = weights[f'layers.{i}.attn.q_proj.weight']
        state_dict[f'layers.{i}.mha.w_k.weight'] = weights[f'layers.{i}.attn.k_proj.weight']
        state_dict[f'layers.{i}.mha.w_v.weight'] = weights[f'layers.{i}.attn.v_proj.weight']
        state_dict[f'layers.{i}.mha.w_o.weight'] = weights[f'layers.{i}.attn.output_proj.weight']
        
        # 前馈网络权重
        state_dict[f'layers.{i}.ff.w1.weight'] = weights[f'layers.{i}.ffn.w1.weight']
        state_dict[f'layers.{i}.ff.w2.weight'] = weights[f'layers.{i}.ffn.w2.weight']
        state_dict[f'layers.{i}.ff.w3.weight'] = weights[f'layers.{i}.ffn.w3.weight']
        
        # 层归一化权重（假设RMSNorm使用'gain'参数）
        state_dict[f'layers.{i}.rmsnorm1.gain'] = weights[f'layers.{i}.ln1.weight']
        state_dict[f'layers.{i}.rmsnorm2.gain'] = weights[f'layers.{i}.ln2.weight']
    
    state_dict['norm.gain'] = weights['ln_final.weight']
...
def run_rmsnorm(
    ...
    output = RMSNorm(d_model, eps)
    state = {'gain': weights}
    output.load_state_dict(state)
    return output(in_features)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    from cs336_basics.model import softmax_
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    # from cs336_basics.train_bpe_tokenizers import train_bpe
